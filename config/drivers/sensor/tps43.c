...
static void tps43_poll_handler(struct k_timer *timer)
{
    struct tps43_data *data = CONTAINER_OF(timer, struct tps43_data, poll_timer);
...
static void tps43_work_handler(struct k_work *work)
{
	...
	ret = i2c_reg_read_byte_dt(&config->i2c, TPS43_REG_TOUCH_STATUS, &status);
	if (ret < 0) {
		LOG_ERR("Failed to read status register: %d", ret);
		return;
	}

	if (status & TPS43_TOUCH_EVENT) {
		/* Read touch data */
		uint8_t xy_info[2];
		ret = i2c_burst_read_dt(&config->i2c, TPS43_REG_XY_INFO_0, xy_info, sizeof(xy_info));
		if (ret < 0) {
		...
		if (data->num_touches > CONFIG_ZMK_SENSOR_TPS43_MAX_TOUCHES) {
			data->num_touches = CONFIG_ZMK_SENSOR_TPS43_MAX_TOUCHES;
		}
...
static void tps43_ready_callback(const struct device *gpio_dev,
				 struct gpio_callback *cb, uint32_t pins)
{
	struct tps43_data *data = CONTAINER_OF(cb, struct tps43_data, ready_cb);

	k_work_submit(&data->work);
}

static int tps43_reset_device(const struct device *dev)
{
	const struct tps43_config *config = dev->config;
	int ret;
...
static int tps43_configure_device(const struct device *dev)
{
	...
	/* Basic configuration for IQS572 */
	uint8_t prox_settings[] = {0x01, 0x00}; /* Enable proximity */
	ret = i2c_burst_write_dt(&config->i2c, TPS43_REG_PROX_SETTINGS, 
	...
	uint8_t thresholds[] = {CONFIG_ZMK_SENSOR_TPS43_SENSITIVITY, 0x08}; /* Touch threshold, proximity threshold */
	ret = i2c_burst_write_dt(&config->i2c, TPS43_REG_THRESHOLDS, 
...
int tps43_init(const struct device *dev)
{
	...
	k_work_init(&data->work, tps43_work_handler);
    k_timer_init(&data->poll_timer, tps43_poll_handler, NULL);
	...
	if (gpio_is_ready_dt(&config->ready_gpio)) {
		...
		gpio_init_callback(&data->ready_cb, tps43_ready_callback, 
        ...
        k_timer_start(&data->poll_timer, K_MSEC(CONFIG_ZMK_SENSOR_TPS43_POLL_RATE_MS), K_MSEC(CONFIG_ZMK_SENSOR_TPS43_POLL_RATE_MS));
    }
...
int tps43_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	return k_sem_take(&data->sem, K_MSEC(CONFIG_ZMK_SENSOR_TPS43_POLL_RATE_MS));
}
...
#define TPS43_INIT(inst)						\
	...
	static const struct tps43_config tps43_config_##inst = {	\
		...
		.max_x = DT_INST_PROP_OR(inst, max_x, TPS43_MAX_X),	\
		.max_y = DT_INST_PROP_OR(inst, max_y, TPS43_MAX_Y),	\
		.max_touch_points = DT_INST_PROP_OR(inst, max_touch_points, TPS43_MAX_TOUCH_POINTS), \
	};								\
									\
